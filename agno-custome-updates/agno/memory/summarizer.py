...
class MemorySummarizer(BaseModel):
    ...
    def update_model(self) -> None:
        # If a model was explicitly provided, just use it
        if self.model is not None:
            pass  # Keep the provided model
        else:
            try:
                import os
                ...
                ollama_model_id = os.environ.get("OLLAMA_MODEL_ID", "llama3.2:latest")
                
                self.model = Ollama(id=ollama_model_id)
                logger.info(f"Using Ollama model {ollama_model_id} for memory summarization")
                
            except (ModuleNotFoundError, ImportError) as e:
                logger.exception(e)
                logger.error(
                    "Could not import Ollama model. Please provide a `model` or ensure Ollama is properly installed."
                )
                exit(1)
