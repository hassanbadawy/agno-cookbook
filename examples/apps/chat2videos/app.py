        ...
        if input_url not in st.session_state.loaded_urls:
            ...
            if input_url.lower().endswith(".pdf"):
                try:
                    ...
                    docs = []
            elif "youtube" in input_url.lower() or "youtu.be" in input_url.lower():
                try:
                    logger.info("Processing YouTube link...")
                    docs: List[Document] = youtube_parser(input_url)
                    logger.info("YouTube link processed successfully")
                    logger.info(docs)
                except Exception as e:
                    st.sidebar.error(f"Error processing YouTube link: {str(e)}")
                    docs = []
            else:
